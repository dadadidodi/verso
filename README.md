...
verso 是个浪漫的词。它是书页的背面。对页也是个浪漫的词。两页相对，读者可以在其中往返。

翻译是件浪漫的事。译文是原著经过译者理解后的样子。但有时候，我们也想回到原文，看看它最初的样子。

verso（对页）就是为这个时刻做的：平时读译文，需要时回到原文。

## 图文版「对页 verso」使用指南

第一次了解 verso，建议先看这份图文入口：[打开图文版使用指南](docs/USER_GUIDE.md)。它用实际宽版截图说明阅读模式、校对模式，以及如何从上传书籍、修正对齐到导出 Reader。

## 它能做什么

- 在本地上传两个不同版本 EPUB（如译文和原文），创建一个项目。
- 用「校对模式」检查章节配对、生成段落对齐、添加「固定对应」修正错配。
- 用「阅读模式」模拟最终阅读体验：读译文，点击段落查看对应原文。
- 把校对好的内容导出成只读静态 Reader 网站，可分享阅读。
...
- verso 工作台：私有制作工具，运行在 `localhost`，包含书库、阅读模式、校对模式、上传、删除、章节配对、段落对齐、固定对应、后台任务和可选 AI 调用。
- 静态 Reader site：从本地项目导出的只读网站，供别人阅读，有轻密码、章节阅读和点击查原文，没有上传、删除、校对、AI 调用或数据库。
...
如果需要 AI 辅助章节配对或更好的段落对齐，在本地 `.env` 放入：
